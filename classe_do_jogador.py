...
art= art_ascii()
class jogador:
    def __init__(self, nome, hp_max, atk, niv, xp_max, defesa, gold, stm_max, intt, mn_max, d_m, buff,art_player,mana_lit=None):
        self.nome = nome
        self.hp_max = hp_max
        self.hp = self.hp_max
    ...
    def load_game(cls, filename="Demo.json"):
        ...
        try:
            ...
            # Carrega o inventário de nomes de volta para objetos Item
            player.inventario = [TODOS_OS_ITENS[nome] for nome in player_data["inventario"]]
            # Carrega os equipamentos de nomes de volta para objetos Item
    ...
    def status(self, x_janela, y_janela):
        draw_window(term, x=x_janela, y=y_janela, width=31, height=7)
        ...
        with term.location(x=x_janela+1, y=y_janela+4):
            print(f"MG: [{term.bold_red(str(self.atk+3))}-{term.red(str(self.atk-3))}] MA: [{term.bold_cyan(str(self.defesa+3))}-{term.cyan(str(self.defesa-3))}]")    
        with term.location(x=x_janela+1, y=y_janela+5):
            input(">")
    #status do jogador
    def status_batalha_art(self, x_janela, y_janela):
        art_player = self.art_player
        num_linha = art_player.count("\n")
        herd = num_linha + 2
    ...
    def status_batalha(self, x_janela, y_janela):
        ...
        with term.location(x=x_janela+1, y=y_janela+2):
            print(f"MG: [{term.bold_magenta(str(self.mana_max))}/{term.magenta(str(self.mana))}] MA: [{term.bold_purple(str(self.dano_magico+3))}-{term.purple(str(self.dano_magico-3))}]")
        with term.location(x=x_janela+1, y=y_janela+3):
            print(f"AT: [{term.bold_red(str(self.atk+3))}-{term.red(str(self.atk-3))}] DS: [{term.bold_cyan(str(self.defesa+3))}-{term.cyan(str(self.defesa-3))}]")
        with term.location(x=x_janela+1, y=y_janela+4):
    ...
    def menu_magias(self, x_menu, y_menu, batalha):
        text_content = ""
        ...
        if escolha in nome_das_magias:
            ...
            if magia_escolhida.batalhas and not batalha:
                ...
                return self.usar_magia(magia_escolhida, x_menu, y_menu + herd)
        else:
    ...
    def usar_magia(self, magia, x_janela, y_janela):
        text_content = ""
        sucesso = False       
        if self.mana < magia.mana_gasta:
            ...
            if magia.tipo == "Cura":
                ...
                text_content = f"Você lançou {magia.nome} e causou {magia.bonus_atk} de dano."
                sucesso = True
...
if __name__ == "__main__":
    jj = jogador(nome="JJ", hp_max=100, atk=10, niv=1, xp_max=100, defesa=10, gold=0, stm_max=100, intt=10, mn_max=100,d_m=20, buff=0, art_player=art.guerriro)
